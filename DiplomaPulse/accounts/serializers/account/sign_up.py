...
from accounts.serializers.errors.account.sign_up import RegistrationCodeNotFound
...
class SignUpSerializer(serializers.Serializer):
    email = serializers.EmailField(
        required=True,
        max_length=BaseUser._meta.get_field("email").max_length,  # noqa: SLF001
    )
    password = serializers.CharField(required=True, min_length=8)
    registration_code = serializers.CharField(required=True)

    def validate(self, attrs: dict) -> ValidatedData:
